...
print(f"\n技能目录: {local_skills_path}")
print(f"目录存在: {os.path.exists(local_skills_path)}")

# 加载本地技能 - 直接读取 .md 文件
skills_dir = os.path.join(local_skills_path, "skills")
print(f"\n技能文件目录: {skills_dir}")
print(f"目录存在: {os.path.exists(skills_dir)}")
        ...
        for filename in os.listdir(skills_dir):
            if filename.endswith('.md') and filename != 'README.md':
                ...
                try:
                    ...
                    print(f"  ✅ 成功加载: {skill.name}")
                except Exception as e:
                    print(f"  ❌ 加载失败: {e}")
                    import traceback
                    traceback.print_exc()
    
    print(f"\n✅ 总共成功加载 {len(local_skills)} 个技能:")
...
# 测试 5: 测试联网搜索技能 (searchtest1.md)
print("\n【测试 5】测试联网搜索技能 - 触发词 '搜索'")
print("-" * 60)
print("发送消息: '搜索今天北京天气怎么样'")
print("预期: Agent 应该通过 Dify API 执行联网搜索并返回结果")
conversation.send_message("搜索今天北京天气怎么样")
conversation.run()

print("\n【测试 6】测试联网搜索技能 - 触发词 '查询'")
print("-" * 60)
print("发送消息: '查询最新的AI新闻'")
print("预期: Agent 应该执行联网搜索")
conversation.send_message("俄乌冲突")
conversation.run()

# 测试 7: 测试时间查询技能 (timetest1.md)
print("\n【测试 7】测试时间查询技能 - 触发词 'time'")
print("-" * 60)
print("发送消息: 'time'")
...
for i, msg in enumerate(llm_messages[-3:], 1):  # 显示最后3条
    ...
    if hasattr(msg, 'role') and msg.role == 'assistant':
        ...
        if "搜索" in full_content or "查询" in full_content or "天气" in full_content:
            print("  ✅ 似乎触发了 searchtest1.md 搜索技能")
...
print("  4. ✅ 支持中英文触发词")
print("  5. ✅ 普通对话不受影响")
print("  6. ✅ 联网搜索技能 (searchtest1.md) 可以通过 Dify API 执行搜索")
print("  7. ✅ 时间查询技能 (timetest1.md) 可以被触发并执行 Python 函数")